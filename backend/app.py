...
from services import start_service, stop_service, register_service, check_service_health
...
async def service_stats(service_name: str, user=Depends(get_current_user)):
    ...
    return await check_all_services_health(service_name)
...
async def service_health(service_name: str, user=Depends(get_current_user)):
    ...
    if user["role"] != "admin":
        raise HTTPException(
            status_code=403, detail="Only admins can view service health"
        )
